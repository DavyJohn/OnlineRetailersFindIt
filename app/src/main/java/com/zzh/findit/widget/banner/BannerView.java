...
public class BannerView extends RelativeLayout implements BannerAdapter.ViewPagerOnItemClickListener
{
    ...
    private List<BannerMode.BannerData.AdData> bannerList;
     ...
     * 图片轮播需要传入参数 url
     */
    public void build(List<BannerMode.BannerData.AdData> list)
    {
        destory();
        ...
        {hImageView = new ImageView(context);
            //TODO 获取图
            url = bannerList.get(i).getAtturl();
        ...
        bannerAdapter = new BannerAdapter(imageViewList,context);
    ...
    {
        if (!bannerList.get(position).getUrl().equals("#")){
            Intent intent = new Intent(context, WebJS.class);
            SharedPreferencesUtil.getInstance(context).putString("url", Contants.BASEURL+bannerList.get(position).getUrl());
